...
# ========= NEW: 中心剪切工具函数 =========
def center_crop(arr, size_y=None, size_x=None):
    """Center-crop 2D array to size_y x size_x.

    - 如果 size_y 或 size_x 为 None / <=0 或大于图像尺寸，则退化为不裁剪该方向。
    - 只支持 2D 数据（ny, nx）。
    """
    arr = np.asarray(arr)
    ny, nx = arr.shape[:2]
    ...
    # 如果都没指定，直接返回原图
    if (size_y is None or size_y <= 0) and (size_x is None or size_x <= 0):
        return arr

    # 默认：没指定就用原尺寸
    if size_y is None or size_y <= 0:
        size_y = ny
    if size_x is None or size_x <= 0:
        size_x = nx

    size_y = int(min(size_y, ny))
    size_x = int(min(size_x, nx))
...
def main():
    ...
    parser.add_argument('--crop_size', type=int, default=None,
                        help='If set, crop object-domain outputs (amp/phase and extra npy) '
                             'to a central square of this side length (pixels).')
    # ========= 新增：矩形裁剪（高 & 宽） =========
    parser.add_argument('--crop_height', type=int, default=None,
                        help='Center crop height in pixels (object domain).')
    parser.add_argument('--crop_width', type=int, default=None,
                        help='Center crop width in pixels (object domain).')
    # =================================================
    ...
    # ========= 统一确定裁剪尺寸 (h, w) =========
    crop_size   = args.crop_size      # 老的方形裁剪
    crop_height = args.crop_height    # 新：高
    crop_width  = args.crop_width     # 新：宽

    use_square = crop_size is not None and crop_size > 0
    if use_square:
        h = w = crop_size
    else:
        h = crop_height
    ...
    def need_crop():
        """是否需要裁剪（任一维度指定了合法尺寸就裁）。"""
        return use_square or ((h is not None and h > 0) or (w is not None and w > 0))
    # =======================================
    ...
    # ========= NEW: 保存剪切后的复场（只剪 object domain） =========
    if need_crop():
        obj_rec_raw_c = center_crop(obj_rec_raw, h, w)
        obj_rec_c     = center_crop(obj_rec,     h, w)
        np.save(os.path.join(args.output_dir, 'reconstructed_object_complex_raw_cropped.npy'),
                obj_rec_raw_c.astype(np.complex64))
        np.save(os.path.join(args.output_dir, 'reconstructed_object_complex_cropped.npy'),
                obj_rec_c.astype(np.complex64))
    # ===============================================================
    ...
    if not args.no_plots:
        ...
        # Target Fourier log-amplitude (centered) —— 频域不剪切
        ...
        # Reconstructed Fourier log-amplitude (centered) —— 频域不剪切
        ...
        if need_crop():
            amp_target_plot = center_crop(amp_target, h, w)
            amp_rec_plot    = center_crop(amp_rec,    h, w)
        else:
            amp_target_plot = amp_target
            amp_rec_plot    = amp_rec

        amp_vmin, amp_vmax = 0.0, 3.0
        save_image_with_colorbar(
            os.path.join(args.output_dir, 'target_object_amplitude.png'),
            amp_target_plot, vmin=amp_vmin, vmax=amp_vmax, cmap='viridis'
        )
        save_image_with_colorbar(
            os.path.join(args.output_dir, 'reconstructed_object_amplitude.png'),
            amp_rec_plot, vmin=amp_vmin, vmax=amp_vmax, cmap='viridis'
        )
        ...
        if target_complex is not None:
            ...
            if need_crop():
                target_phase_plot = center_crop(target_phase, h, w)
                phase_rec_plot    = center_crop(phase_rec,    h, w)
            else:
                target_phase_plot = target_phase
                phase_rec_plot    = phase_rec

            # 你这里固定了相位范围 [-3.15, 3.15]，我保持你的设定
            phase_min = -3.15
            phase_max =  3.15

            save_image_with_colorbar(
                os.path.join(args.output_dir, 'target_object_phase_from_file.png'),
                target_phase_plot, vmin=phase_min, vmax=phase_max, cmap='twilight'
            )
            save_image_with_colorbar(
                os.path.join(args.output_dir, 'reconstructed_object_phase.png'),
                phase_rec_plot, vmin=phase_min, vmax=phase_max, cmap='twilight'
            )
        else:
            if need_crop():
                phase_rec_plot = center_crop(phase_rec, h, w)
    ...
    # Validation against target complex field（完整尺寸的评估不变）
    if target_complex is not None:
